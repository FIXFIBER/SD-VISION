...
Your colleagues will need to pull the changes and run `npm run migrate:local` to apply the migration in their local database.
...
```bash
docker build -t SD-VISION-image .
docker run -p 3500:3500 -e ADMINFORTH_SECRET=123 SD-VISION-image
```
